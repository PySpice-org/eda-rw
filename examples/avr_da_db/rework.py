...
def make_base_device(family, device, package):
    ...
    def render_side_banks(banks, x, angle):
        ...
        for bank in banks:
            for pin in bank:
                render_pin(directionality(pin), make_pin_label(pin), pin.number, x, y, angle)
                y -= grid
            y -= grid
            ...
            for alias_device in family_devices_with_pin_count:
                for package in packages_with_no_exposed_pad:
                    if (
                        ...
                        make_alias_device(family, alias_device, package, base_with_no_exposed_pad_info)
            ...
            for alias_device in family_devices_with_pin_count:
                for package in packages_with_exposed_pad:
                    if (
                        ...
                        make_alias_device(family, alias_device, package, base_with_exposed_pad_info)
